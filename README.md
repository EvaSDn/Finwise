...
Application web complète : portefeuille virtuel sur cours réels, workflow budget façon Finary, plan de versements DCA, alertes de concentration, actualités financières multi-catégories et agent IA.
Voir AUDIT.md pour l'audit technique complet, la feuille de route détaillée par chantier et le guide de déploiement.
Démarrage rapide
npm install
...
server/
  ...
  services/
    ...
    news.js           Actualités multi-catégories (marchés, actions, ETF,
                      obligations, matières premières, devises, crypto,
                      entreprises) via Finnhub + flux personnalisé sur les
                      positions détenues, cache 5 min, mode démo complet
    agent.js          Analyse déterministe (concentration, risque en €) +
                      Claude/Gemini avec garde-fous, fallback règles
public/
